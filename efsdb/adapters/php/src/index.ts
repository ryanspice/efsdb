...
export default function adapterEfsdbPhp(opts: AdapterEfsdbPhpOptions = {}): Adapter {
  ...
  return {
    ...
    async adapt(builder) {
      ...
      builder.log.minor("Writing PHP runtime...");
      // CHANGED: Runtime is now in ../../../php/runtime relative to this source file
      // If built to dist/index.js, it's relative to dist.
      // But we are running from source or dist? usually dist.
      // In monorepo, we can try to resolve it relative to package root.
      // But let's assume standard build.
      // If we are in src/index.ts, runtime is at ../../../php/runtime
      
      const runtimeSrc = path.resolve(path.dirname(fileURLToPath(import.meta.url)), "../../../php/runtime");
      
      // Copy runtime files
      if (fs.existsSync(runtimeSrc)) {
           copyDir(runtimeSrc, publicDir);
      } else {
           builder.log.warn(`Runtime not found at ${runtimeSrc}. You may need to copy it manually.`);
      }
      ...
      if (o.runPhpImport) {
        ...
        const buildImportArgs = (sourceDir: string, extra: string[] = []) => {
          ...
          pushCliOption(args, "root", "published");
        ...
        if (o.encryptAppAssets && clientTmp) {
          ...
          await execPhp(o.phpBin!, buildImportArgs(clientTmp, [
            `--prefix=${builder.config.kit.appDir}`
          ]));
        }

        builder.log.minor("Importing prerendered files into EFSDB...");
        await execPhp(o.phpBin!, buildImportArgs(tmp));
      }
    }
  };
}
