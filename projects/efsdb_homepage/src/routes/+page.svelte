<script lang="ts">
	...
	// On server in production build, use placeholder for PHP replacement.
	// In dev mode, or on the client, use the actual base path.
	let basePath = $state(
		dev
			? ''
			: (browser && typeof window !== 'undefined' && window.__EFSDB__
				? (window.__EFSDB__.basePath ?? '')
				: '__EFSDB_BASE_PATH__')
	);
	...
	onMount(() => {
		...
		if (typeof window !== 'undefined' && window.__EFSDB__) {
			console.log('[EFSDB] Hydration Data:', window.__EFSDB__);
			// Ensure it's up to date in case the client value differs
			basePath = window.__EFSDB__.basePath ?? '';
			efsdbData = window.__EFSDB__;
		} else {
			console.log('[EFSDB] No window.__EFSDB__ found during hydration');
		}
	});

	// Example of fetching public data from EFSDB
	// In a real scenario, this would fetch from /api/public/... or load from local content files
	let siteData = $state({
		title: "EFSDB Public Template",
