...
Still present but intentionally non-authoritative:

- `spec/characterization/ui/ExplorerBehaviorCharacterization.spec.ts`
  - this remains the earlier Phase 0 placeholder and is not the active executable parity suite
...
5. Replace the old `ExplorerBehaviorCharacterization.spec.ts` placeholder with real executable parity checks when explorer interaction parity becomes the active goal.

See `spec/phase3/cleanup-plan.md` for removal criteria and sequencing.
